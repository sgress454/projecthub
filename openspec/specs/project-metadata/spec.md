...
Defines the per-project metadata attached to each ProjectHub project beyond the base name/space pairing: GitHub issue URLs, GitHub PR URLs (manual and auto-discovered), arbitrary labeled links, and an optional OpenSpec change association. Also defines the editing modal through which the user manages this metadata.

# fleet-process-indicators Specification

## Purpose

Surfaces "is a Fleet server / webpack build running, and which project owns it" in the menu bar so the user does not have to hunt through Spaces and iTerm windows to find live processes. Pairs with a user-configured iTerm hotkey-window keystroke so that clicking an indicator brings the dedicated Fleet console to the front from any Space.
...
For each detected process, the app SHALL attribute the process to the configured project whose `path` is the longest prefix of the process's current working directory. If no project's `path` is a prefix of the cwd, the process SHALL NOT be attributed to any project.

The webpack `--output-path` (or its synonyms `-o` / `--output`) is independent of attribution: it determines what the 🎨 hover tooltip displays (see "Hover detail for process indicators"), not which project owns the build. Attribution always follows the cwd of the running process.
...
#### Scenario: Webpack attributed by cwd, output-path is for tooltip only

- **GIVEN** project "server" has `path` = `/Users/scott/code/server`
- **AND** project "frontend" has `path` = `/Users/scott/code/frontend`
- **AND** a process running `yarn run webpack --progress --output-path /Users/scott/code/server/server/assets` was launched with cwd `/Users/scott/code/frontend`
- **WHEN** the menu bar list is rendered
- **THEN** the frontend project's row shows a 🎨 indicator
- **AND** the server project's row does NOT show a 🎨 indicator
- **AND** the 🎨 tooltip on the frontend row shows `/Users/scott/code/server/server/assets`

#### Scenario: Webpack without an output flag
...
- 🎨 (Webpack): the absolute path of the explicit output directory if a webpack output-directory flag was supplied (`--output-path`, `-o`, or `--output`), otherwise the absolute path of the project's directory.
